(ns hazmat.operation
  "OperationActor — one intake/manifest/disposal-coordination/concern operation
  = one supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (HazmatDispatch-LLM) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the HazardousWasteGovernor (:govern)
  ...
  Human-in-the-loop = real approval / escalation workflow:
  `interrupt-before #{:request-approval}` pauses the actor and hands the
  decision to a human reviewer (dispatch coordinator / hazard inspector).
  The reviewer resumes with `{:approval {:status :approved}}` (or `:rejected`)."
...
(defn build
  ...
  `hazmat.store/Store`).
  opts:
    :advisor      — a `hazmat.llm/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; HazmatDispatch-LLM inference (the contained intelligence node) — proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (llm/-advise advisor store request)]
            {:proposal p :audit [(llm/trace request p)]})))

      ;; HazardousWasteGovernor — independent censor (separate system than the LLM).
      (g/add-node :govern
          ...
          (let [base (phase/verdict->disposition verdict)
            ...
            (case disposition
              ...
              {:disposition :escalate
               :audit [{:t :escalation-required
                        :op (:op request) :subject (:subject request)
                        :reason (or reason
                                    (cond (:hazard-concern? verdict) :hazard-concern-escalation
                                          (:bulk? verdict) :bulk-intake
                                          :else :low-confidence))
                        :phase ph
      ...
      ;; (dispatch coordinator / hazard inspector) resumes with :approval.
